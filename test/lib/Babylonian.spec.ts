...
import BabylonianTest from "../../artifacts/contracts/lib/test/BabylonianTest.sol/BabylonianTest.json";
...
describe("Babylonian", () => {
  ...
  describe("#sqrt", () => {
    ...
    it("gas cost of large number", async () => {
      expect(
        await babylonian.getGasCostOfSqrt(BigNumber.from(2).pow(150))
      ).to.eq(720);
    });

    it("gas cost of max uint", async () => {
      expect(
        await babylonian.getGasCostOfSqrt(BigNumber.from(2).pow(256).sub(1))
      ).to.eq(798);
    });
  });
});
