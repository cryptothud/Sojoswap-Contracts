...
import FixedPointTest from "../../artifacts/contracts/lib/test/FixedPointTest.sol/FixedPointTest.json";
import { deployContract, myProvider, solidity } from "../reexports";
...
describe("FixedPoint", () => {
  ...
  describe("#encode", () => {
    ...
    it("will not take >uint112(-1)", async () => {
      expect(() => fixedPoint.encode(BigNumber.from(2).pow(113).sub(1))).to
        .throw;
    });
  });
  ...
  describe("#encode144", () => {
    ...
    it("will not take >uint144(-1)", async () => {
      expect(() => fixedPoint.encode144(BigNumber.from(2).pow(145).sub(1))).to
        .throw;
    });
  });
  ...
  describe("#decode", () => {
    it("shifts right by 112", async () => {
      expect(await fixedPoint.decode([BigNumber.from(3).mul(Q112)])).to.eq(
        BigNumber.from(3)
      );
    });
    it("will not take >uint224(-1)", async () => {
      expect(() => fixedPoint.decode([BigNumber.from(2).pow(225).sub(1)])).to
        .throw;
    });
  });
  ...
  describe("#decode144", () => {
    ...
    it("will not take >uint256(-1)", async () => {
      expect(() => fixedPoint.decode([BigNumber.from(2).pow(257).sub(1)])).to
        .throw;
    });
  });
  ...
  describe("#mul", () => {
    ...
    it("correct multiplication", async () => {
      expect(
        (
          await fixedPoint.mul([BigNumber.from(3).mul(Q112)], BigNumber.from(2))
        )[0]
      ).to.eq(BigNumber.from(3).mul(2).mul(Q112));
    });
    ...
    it("overflow", async () => {
      ...
      ).to.be.revertedWith("FixedPoint::mul: overflow");
    });

    it("max of q112x112", async () => {
      expect(
        (
          await fixedPoint.mul(
            [BigNumber.from(2).pow(112)],
            BigNumber.from(2).pow(112)
          )
        )[0]
      ).to.eq(BigNumber.from(2).pow(224));
    });
    ...
    it("max without overflow, smallest fixed point", async () => {
      ...
      expect((await fixedPoint.mul([BigNumber.from(1)], maxUint))[0]).to.eq(
        maxUint
      );
      await expect(
        fixedPoint.mul([BigNumber.from(2)], maxUint)
      ).to.be.revertedWith("FixedPoint::mul: overflow");
    });
  });
  ...
  describe("#muli", () => {
    it("works for 0", async () => {
      expect(
        await fixedPoint.muli([BigNumber.from(0).mul(Q112)], BigNumber.from(1))
      ).to.eq(BigNumber.from(0));
      expect(
        await fixedPoint.muli([BigNumber.from(1).mul(Q112)], BigNumber.from(0))
      ).to.eq(BigNumber.from(0));
    });

    it("works for 3*2", async () => {
      expect(
        await fixedPoint.muli([BigNumber.from(3).mul(Q112)], BigNumber.from(2))
    ...
    it("max without overflow, largest int", async () => {
      ...
      expect(
        await fixedPoint.muli([BigNumber.from(1).mul(Q112)], maxInt)
      ).to.be.eq(maxInt);

      const minInt = BigNumber.from(2).pow(255).mul(-1);
      await expect(
        fixedPoint.muli([BigNumber.from(1).mul(Q112)], minInt)
      ).to.be.revertedWith("FixedPoint::muli: overflow");

      expect(
        await fixedPoint.muli([BigNumber.from(1).mul(Q112).sub(1)], minInt)
      ).to.be.eq(
        "-57896044618658097711785492504343942776262393067508711251869655679775811829760"
      );
      expect(
        await fixedPoint.muli([BigNumber.from(1).mul(Q112)], minInt.add(1))
    ...
    it("max without overflow, largest fixed point", async () => {
      ...
      expect(
        await fixedPoint.muli(
          [BigNumber.from(2).pow(224).sub(1)],
          maxMultiplier
        )
      ).to.eq(
        BigNumber.from(
          "57896044618658097711785492504343953926634992332820282019728792003954417336320"
        )
      );
      await expect(
        fixedPoint.muli(
  ...
  describe("#muluq", () => {
    it("works for 0", async () => {
      expect((await fixedPoint.muluq([BigNumber.from(0)], [Q112]))[0]).to.eq(
        BigNumber.from(0)
      );
      expect((await fixedPoint.muluq([Q112], [BigNumber.from(0)]))[0]).to.eq(
        BigNumber.from(0)
      );
    });
    ...
    it("multiplies 4/3*4/3", async () => {
      ...
      expect((await fixedPoint.muluq([multiplier], [multiplier]))[0]).to.eq(
        expectedResult
      );
    ...
    it("overflow upper", async () => {
      ...
      await expect(
        fixedPoint.muluq([multiplier1], [multiplier2])
      ).to.be.revertedWith("FixedPoint::muluq: upper overflow");
      expect(
        (await fixedPoint.muluq([multiplier1.sub(1)], [multiplier2]))[0]
      ).to.eq(multiplyExpanded(multiplier1.sub(1), multiplier2));
      expect(
        (await fixedPoint.muluq([multiplier1], [multiplier2.sub(1)]))[0]
      ).to.eq(multiplyExpanded(multiplier1, multiplier2.sub(1)));
    });

    it("gas for short circuit where one multiplicand is 0", async () => {
      expect(
        await fixedPoint.getGasCostOfMuluq(
          [BigNumber.from(0)],
  ...
  describe("#divuq", () => {
    it("works for 0 numerator", async () => {
      expect((await fixedPoint.divuq([BigNumber.from(0)], [Q112]))[0]).to.eq(
        BigNumber.from(0)
      );
    });
    ...
    it("throws for 0 denominator", async () => {
      await expect(
        fixedPoint.divuq([Q112], [BigNumber.from(0)])
      ).to.be.revertedWith("FixedPoint::divuq: division by zero");
    });
    ...
    it("divides 30/10", async () => {
      ...
      ).to.eq(BigNumber.from(3).mul(Q112));
    });
    ...
    it("divides 35/8", async () => {
      ...
      ).to.eq(BigNumber.from(4375).mul(Q112).div(1000));
    });
    ...
    it("boundary of full precision", async () => {
      ...
      expect(
        ...
        BigNumber.from(
          "26959946667150639794667015087019630673637143213614752866474435543040"
        )
      );

      await expect(
        fixedPoint.divuq(
          [maxNumeratorFullPrecision.add(1)],
          [minDenominatorFullPrecision]
        )
      ...
      await expect(
        fixedPoint.divuq(
          [maxNumeratorFullPrecision],
          [minDenominatorFullPrecision.sub(1)]
        )
      ).to.be.revertedWith("FixedPoint::divuq: overflow");
    });
    ...
    it("precision", async () => {
      ...
      expect(
        (await fixedPoint.divuq([numerator], [numerator.sub(1)]))[0]
      ).to.eq(BigNumber.from("5192296858534827628530496329220096"));

      expect(
        (await fixedPoint.divuq([numerator], [numerator.add(1)]))[0]
      ).to.eq(BigNumber.from("5192296858534827628530496329220095"));
    });
    ...
    it("divuq overflow with smaller numbers", async () => {
      ...
      await expect(
        fixedPoint.divuq([numerator], [denominator])
      ).to.be.revertedWith("FixedPoint::divuq: overflow");
    });
    ...
    it("gas cost of full precision small dividend short circuit", async () => {
      expect(
        await fixedPoint.getGasCostOfDivuq(
          [BigNumber.from(125).mul(Q112)],
          [BigNumber.from(30).mul(Q112)]
        )
      ).to.eq(838);
      expect(
        await fixedPoint.getGasCostOfDivuq(
          [BigNumber.from(28).mul(Q112)],
          [BigNumber.from(280).mul(Q112)]
        )
      ).to.eq(838);
      expect(
        await fixedPoint.getGasCostOfDivuq(
  ...
  describe("#fraction", () => {
    it("correct computation less than 1", async () => {
      expect((await fixedPoint.fraction(4, 100))[0]).to.eq(
        BigNumber.from(4).mul(Q112).div(100)
      );
    });

    it("correct computation greater than 1", async () => {
      expect((await fixedPoint.fraction(100, 4))[0]).to.eq(
        BigNumber.from(100).mul(Q112).div(4)
      );
    });

    it("fails with 0 denominator", async () => {
      await expect(
        fixedPoint.fraction(BigNumber.from(1), BigNumber.from(0))
      ).to.be.revertedWith("FixedPoint::fraction: division by zero");
    });
    it("can be called with numerator exceeding uint112 max", async () => {
      expect((await fixedPoint.fraction(Q112.mul(2359), 6950))[0]).to.eq(
        Q112.mul(Q112).mul(2359).div(6950)
      );
    });
    ...
    it("can be called with numerator exceeding uint144 max", async () => {
      expect(
        (
          await fixedPoint.fraction(
            Q112.mul(2359).mul(BigNumber.from(2).pow(32)),
            Q112.mul(50)
          )
        )[0]
      ).to.eq(
    ...
    it("can be called with numerator and denominator exceeding uint112 max", async () => {
      expect(
        (await fixedPoint.fraction(Q112.mul(2359), Q112.mul(50)))[0]
      ).to.eq(BigNumber.from(2359).mul(Q112).div(50));
    });
    it("short circuits for 0", async () => {
      expect((await fixedPoint.fraction(0, Q112.mul(Q112).mul(2360)))[0]).to.eq(
        0
      );
    });
    it("can overflow if result of division does not fit", async () => {
      await expect(fixedPoint.fraction(Q112.mul(2359), 50)).to.be.revertedWith(
        "FixedPoint::fraction: overflow"
      );
    });
    it("gas cost of 0", async () => {
      expect(
        await fixedPoint.getGasCostOfFraction(
          BigNumber.from(0),
          BigNumber.from(569)
        )
      ).to.eq(210);
    ...
    it("gas cost of number greater than Q112 numbers", async () => {
      expect(
        await fixedPoint.getGasCostOfFraction(Q112.mul(2359), Q112.mul(2360))
      ).to.eq(314);
    });
    it("gas cost of number greater than Q112 numbers", async () => {
      expect(
        await fixedPoint.getGasCostOfFraction(
          Q112.mul(BigNumber.from(2).pow(32).mul(2359)),
          Q112.mul(2360)
        )
      ).to.eq(996);
    });
  ...
  describe("#reciprocal", () => {
    it("fails for 0", async () => {
      await expect(
        fixedPoint.reciprocal([BigNumber.from(0)])
      ).to.be.revertedWith("FixedPoint::reciprocal: reciprocal of zero");
    });
    it("fails for 1", async () => {
      await expect(
        fixedPoint.reciprocal([BigNumber.from(1)])
      ).to.be.revertedWith("FixedPoint::reciprocal: overflow");
    });
    it("works for 0.25", async () => {
      expect(
        (
          await fixedPoint.reciprocal([Q112.mul(BigNumber.from(25)).div(100)])
        )[0]
      ).to.eq(Q112.mul(4));
    });
    it("works for 5", async () => {
      expect(
        (await fixedPoint.reciprocal([Q112.mul(BigNumber.from(5))]))[0]
      ).to.eq(Q112.mul(BigNumber.from(1)).div(5));
    });
  });

  describe("#sqrt", () => {
    it("works with 0", async () => {
      expect((await fixedPoint.sqrt([BigNumber.from(0)]))[0]).to.eq(
        BigNumber.from(0)
      );
    });
    ...
    it("works with numbers less than 1", async () => {
      expect(
        (await fixedPoint.sqrt([BigNumber.from(1225).mul(Q112).div(100)]))[0]
      ).to.eq(BigNumber.from(35).mul(Q112).div(10));
    });
    ...
    it("works for 25", async () => {
      expect((await fixedPoint.sqrt([BigNumber.from(25).mul(Q112)]))[0]).to.eq(
        BigNumber.from(5).mul(Q112)
      );
    });
    ...
    it("works for encoded max uint112", async () => {
      ...
      const result = (await fixedPoint.sqrt([input]))[0];
      const expected = BigNumber.from(
        "374144419156711147060143317175368417003121712037887"
      );
