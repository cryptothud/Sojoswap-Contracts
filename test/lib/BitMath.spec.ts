...
import BitMathTest from "../../artifacts/contracts/lib/test/BitMathTest.sol/BitMathTest.json";
...
describe("BitMath", () => {
  ...
  describe("#mostSignificantBit", () => {
    ...
    it("gas cost of smaller number", async () => {
      expect(
        await bitMath.getGasCostOfMostSignificantBit(BigNumber.from(3568))
      ).to.eq(295);
    });
    it("gas cost of max uint128", async () => {
      expect(
        await bitMath.getGasCostOfMostSignificantBit(
  ...
  describe("#leastSignificantBit", () => {
    it("0", async () => {
      await expect(bitMath.leastSignificantBit(0)).to.be.revertedWith(
        "BitMath::leastSignificantBit: zero"
      );
    });
    it("1", async () => {
    ...
    it("gas cost of smaller number", async () => {
      expect(
        await bitMath.getGasCostOfLeastSignificantBit(BigNumber.from(3568))
      ).to.eq(408);
    });
    it("gas cost of max uint128", async () => {
      expect(
        await bitMath.getGasCostOfLeastSignificantBit(
          BigNumber.from(2).pow(128).sub(1)
        )
      ).to.eq(407);
    });
