...
import TransferHelperTest from "../../artifacts/contracts/lib/test/TransferHelperTest.sol/TransferHelperTest.json";
import FakeFallback from "../../artifacts/contracts/lib/test/TransferHelperTest.sol/TransferHelperTestFakeFallback.json";
import FakeERC20Compliant from "../../artifacts/contracts/lib/test/TransferHelperTest.sol/TransferHelperTestFakeERC20Compliant.json";
import FakeERC20Noncompliant from "../../artifacts/contracts/lib/test/TransferHelperTest.sol/TransferHelperTestFakeERC20Noncompliant.json";
...
describe("TransferHelper", () => {
  ...
  before(async () => {
    transferHelper = await deployContract(
      wallet,
      TransferHelperTest,
      [],
      overrides
    ...
    fakeNoncompliant = await deployContract(
      wallet,
      FakeERC20Noncompliant,
      [],
      overrides
    );
    fakeCompliant = await deployContract(
      wallet,
      FakeERC20Compliant,
      [],
      overrides
  ...
  function harness({
    ...
    it("fails with compliant with no revert and false return", async () => {
      ...
      await expect(sendTx(fakeCompliant.address)).to.be.revertedWith(
        expectedError
      );
    });
    ...
    it("fails with noncompliant (no return) with revert", async () => {
      ...
      await expect(sendTx(fakeNoncompliant.address)).to.be.revertedWith(
        expectedError
      );
    });
  }
  ...
  describe("#safeTransferETH", () => {
    ...
    it("fails if call reverts", async () => {
      ...
      await expect(
        transferHelper.safeTransferETH(fakeFallback.address, 0)
      ).to.be.revertedWith(
