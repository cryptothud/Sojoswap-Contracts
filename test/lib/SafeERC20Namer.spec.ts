...
import SafeERC20NamerTest from "../../artifacts/contracts/lib/test/SafeERC20NamerTest.sol/SafeERC20NamerTest.json";
import FakeCompliantERC20 from "../../artifacts/contracts/lib/test/SafeERC20NamerTest.sol/NamerTestFakeCompliantERC20.json";
import FakeNoncompliantERC20 from "../../artifacts/contracts/lib/test/SafeERC20NamerTest.sol/NamerTestFakeNoncompliantERC20.json";
import FakeOptionalERC20 from "../../artifacts/contracts/lib/test/SafeERC20NamerTest.sol/NamerTestFakeOptionalERC20.json";
  ...
  describe("#tokenName", () => {
    ...
    it("works with empty bytes32", async () => {
      ...
      expect(await getName(token.address)).to.eq(
        token.address.toUpperCase().substr(2)
      );
    });
    ...
    it("works with non-code address", async () => {
      expect(await getName(constants.AddressZero)).to.eq(
        constants.AddressZero.substr(2)
      );
    });
  ...
  describe("#tokenSymbol", () => {
    ...
    it("works with empty bytes32", async () => {
      ...
      expect(await getSymbol(token.address)).to.eq(
        token.address.substr(2, 6).toUpperCase()
      );
    });
    it("works with noncompliant full bytes32", async () => {
      const token = await deployNoncompliant({
    ...
    it("works with non-code address", async () => {
      expect(await getSymbol(constants.AddressZero)).to.eq(
        constants.AddressZero.substr(2, 6)
      );
    });
    it("works with really long strings", async () => {
      const token = await deployCompliant({
